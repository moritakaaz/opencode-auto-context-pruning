...
`@moritakaaz/opencode-apc` — an opencode plugin that wraps `@tarquinen/opencode-dcp` with aggressive auto-compression defaults (7k token trigger). It ensures DCP config files exist before delegating all hook behavior to DCP.
...
Config is never overwritten if it already exists **and has no `_apcVersion` field** — user preferences always win.
Configs generated by APC (containing `_apcVersion`) are auto-upgraded when the plugin ships a newer version.
...
| Setting | Value | Why |
|---------|-------|-----|
| maxContextLimit | 7000 | Triggers compression early to save tokens |
